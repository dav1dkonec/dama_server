...
//                  nebo ID;ERROR;ROOM_NOT_FOUND|NOT_LOGGED_IN|ROOM_FULL
// Když se room naplní:
...
rn::PLAYER2 && playerIndex != 1)) {
    ...
    if (isCapture && canCaptureFrom(room, toRow, toCol, placed)) {
...
void checkTimeouts(
    ...
    for (auto& [roomId, room] : rooms) {
        ...
        if (elapsed > turnTimeoutMs) {
            ...
            sendGameEnd(0, room, players, sockfd, "TURN_TIMEOUT");
            resetRoom(room);
        }
    }
}
